...
class ErrorHandler(QtCore.QObject):
    """
    Helper class to handle exceptions on the main thread.
    Required on macOS where GUI operations must happen on the main thread.
    """
    ...
    def __init__(self):
        super().__init__()
        # Use QueuedConnection to ensure the slot runs on the main thread
        # even when the signal is emitted from a background thread
        self.error_occurred.connect(
            self.show_error_dialog, QtCore.Qt.ConnectionType.QueuedConnection
        )

    def show_error_dialog(self, exception_text):
        """Show error dialog on the main thread."""
        errorbox = QtWidgets.QMessageBox()
        errorbox.addButton(
            QtWidgets.QPushButton("Close"), QtWidgets.QMessageBox.ButtonRole.YesRole
        )
        errorbox.addButton(
            QtWidgets.QPushButton("Copy text && Close"),
            QtWidgets.QMessageBox.ButtonRole.NoRole,
        )
        errorbox.setText(exception_text)
        ret = errorbox.exec()
        if ret == 1:
            cb = QtWidgets.QApplication.clipboard()
            cb.clear(mode=cb.Mode.Clipboard)
            cb.setText(exception_text)
...
class BMicro(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        QtCore.QCoreApplication.setOrganizationDomain("bmicro.readthedocs.io")
        ...
        # Initialize the error handler on the main thread
        # This is required for macOS where GUI operations must happen on the main thread
        get_error_handler()
    ...
    def connect_menu(self):
        ...
        self.action_batch_evaluation.triggered.connect(self.on_action_batch_evaluation)
    ...
    def open_file(self, file_name=None):
        ...
        if not file_name:
            ...
            self.settings.setValue(
                "path/last-used", str(pathlib.Path(file_name).parent)
            )
    ...
    def on_action_export_file(self):
        ...
        self.export_dialog.button_export.clicked.connect(self.export_file)
        self.export_dialog.button_cancel.clicked.connect(self.close_export_dialog)
    ...
    def init_export_dialog(self, parent_widget):
        ...
        for key, parameter in parameters.items():
            ...
            # Checkbox
            checkbox = QCheckBox()
            checkbox.setText(parameter["label"] + " [" + parameter["unit"] + "]")
            ...
            checkbox.clicked.connect(
                lambda checked, param=key, minbox=min_box, maxbox=max_box: self.on_export_checkbox(
                    param, minbox, maxbox
                )
            )
    ...
    def on_action_about(self):
        ...
        QtWidgets.QMessageBox.about(self, f"BMicro {bmicroversion}", about_text)
    ...
    def on_action_batch_evaluation(self):
        ...
        self.batch_dialog.setWindowModality(QtCore.Qt.WindowModality.ApplicationModal)
        self.batch_dialog.button_start_cancel.clicked.connect(
            self.start_batch_evaluation
        )
        self.batch_dialog.button_add_folder.clicked.connect(self.batch_add_files)
        self.batch_dialog.button_remove_folder.clicked.connect(self.batch_remove_files)
    ...
    def update_batch_file_settings(self):
        ...
        self.batch_dialog.combobox_setup.addItems([s.name for s in AVAILABLE_SETUPS])
        ...
        self.batch_dialog.checkBox_orientation.setChecked(cfg_orientation["set"])
        self.batch_dialog.checkBox_orientation.clicked.connect(self.on_orientation_set)
        self.batch_dialog.groupBox_rotation.setEnabled(cfg_orientation["set"])
        self.batch_dialog.groupBox_reflection.setEnabled(cfg_orientation["set"])
        ...
        self.batch_dialog.radio_rotation_none.clicked.connect(self.on_rotation_clicked)
        self.batch_dialog.radio_rotation_90_cw.clicked.connect(self.on_rotation_clicked)
        self.batch_dialog.radio_rotation_90_ccw.clicked.connect(
            self.on_rotation_clicked
        )
        ...
        self.batch_dialog.checkBox_extraction.setChecked(cfg_extraction["extract"])
        self.batch_dialog.checkBox_extraction.clicked.connect(self.on_extraction_set)

        # Calibration
        cfg_calibration = self.batch_config["calibration"]
        self.batch_dialog.checkBox_find_peaks.setChecked(cfg_calibration["find-peaks"])
        self.batch_dialog.checkBox_find_peaks.clicked.connect(
            self.on_calibration_find_peaks
        )
        self.batch_dialog.checkBox_calibrate.setChecked(cfg_calibration["calibrate"])
        self.batch_dialog.checkBox_calibrate.clicked.connect(
            self.on_calibration_calibrate
        )

        self.batch_dialog.temperature.valueChanged.connect(self.temperature_changed)
        self.batch_dialog.shift_0.valueChanged.connect(self.shift_methanol_changed)
        self.batch_dialog.shift_1.valueChanged.connect(self.shift_water_changed)

        self.update_batch_calibration_frequencies()

        # Peak selection
        cfg_peak_selection = self.batch_config["peak-selection"]
        self.batch_dialog.checkBox_peak_selection.setChecked(
            cfg_peak_selection["select"]
        )
        self.batch_dialog.checkBox_peak_selection.clicked.connect(
            self.on_peak_selection_select
        )
        self.batch_dialog.tableWidget_Brillouin.setEnabled(cfg_peak_selection["select"])
        self.batch_dialog.tableWidget_Rayleigh.setEnabled(cfg_peak_selection["select"])
        ...
        self.batch_dialog.tableWidget_Brillouin.itemChanged.connect(
            lambda item: self.on_region_changed(
                self.batch_config["peak-selection"]["brillouin_regions"], item
            )
        )

        self.batch_dialog.tableWidget_Rayleigh.itemChanged.connect(
            lambda item: self.on_region_changed(
                self.batch_config["peak-selection"]["rayleigh_regions"], item
            )
        )
        ...
        self.batch_dialog.tableWidget_Brillouin.verticalHeader().setVisible(False)
        self.batch_dialog.tableWidget_Brillouin.setHorizontalHeaderLabels(
            ["start", "end"]
        )
        ...
        header.setSectionResizeMode(0, QtWidgets.QHeaderView.ResizeMode.Stretch)
        header.setSectionResizeMode(1, QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.batch_dialog.tableWidget_Rayleigh.verticalHeader().setVisible(False)
        self.batch_dialog.tableWidget_Rayleigh.setHorizontalHeaderLabels(
            ["start", "end"]
        )

        # Evaluation
        cfg_evaluation = self.batch_config["evaluation"]
        self.batch_dialog.checkBox_evaluation.setChecked(cfg_evaluation["evaluate"])
        self.batch_dialog.checkBox_evaluation.clicked.connect(
            self.on_evaluation_evaluate
        )
        self.batch_dialog.nrBrillouinPeaksGroup.setEnabled(cfg_evaluation["evaluate"])
        ...
        self.batch_dialog.bounds_table.setEnabled(
            cfg_evaluation["evaluate"] and cfg_evaluation["nr_brillouin_peaks"] > 1
        )
        ...
        self.batch_dialog.checkBox_export.setChecked(cfg_export["export"])
        self.batch_dialog.checkBox_export.clicked.connect(self.on_export_export)
    ...
    def shift_methanol_changed(self):
        ...
        self.batch_config["setup"]["setup"].calibration.set_shift_methanol(1e9 * shift)

    def shift_water_changed(self):
        shift = self.sender().value()

        self.batch_config["setup"]["setup"].calibration.set_shift_water(1e9 * shift)
    ...
    def update_batch_bounds_table(self):
        ...
        self.batch_dialog.bounds_table.setEnabled(
            ...
            and self.batch_config["evaluation"]["nr_brillouin_peaks"] > 1
        )
    ...
    def set_nr_brillouin_peaks(self, nr_peaks):
        ...
        if nr_peaks > 1 and (
            self.batch_config["evaluation"]["bounds_w0"] is None
            or len(self.batch_config["evaluation"]["bounds_w0"]) is not nr_peaks
        ):
            bounds_w0 = [["min", "max"] for _ in range(nr_peaks)]
        if nr_peaks > 1 and (
            self.batch_config["evaluation"]["bounds_fwhm"] is None
            or len(self.batch_config["evaluation"]["bounds_fwhm"]) is not nr_peaks
        ):
    ...
    def remove_evaluation_regions(self, region_table, region_list):
        ...
        sorted_ranges = sorted(
            selected_ranges, key=lambda item: item.topRow(), reverse=True
        )
    ...
    def on_setup_set(self):
        ...
        self.batch_dialog.combobox_setup.setEnabled(self.batch_config["setup"]["set"])

        self.update_batch_calibration_frequencies()

    def on_orientation_set(self):
        self.batch_config["orientation"]["set"] = self.sender().isChecked()
        self.batch_dialog.groupBox_rotation.setEnabled(
            self.batch_config["orientation"]["set"]
        )
        self.batch_dialog.groupBox_reflection.setEnabled(
            self.batch_config["orientation"]["set"]
        )
    ...
    def on_reflection_clicked(self):
        ...
        if checkbox == self.batch_dialog.checkbox_reflect_vertically:
            self.batch_config["orientation"]["reflection"][
                "vertically"
            ] = checkbox.isChecked()
    ...
    def on_calibration_find_peaks(self):
        self.batch_config["calibration"]["find-peaks"] = self.sender().isChecked()

    def on_calibration_calibrate(self):
        self.batch_config["calibration"]["calibrate"] = self.sender().isChecked()

    def on_peak_selection_select(self):
        self.batch_config["peak-selection"]["select"] = self.sender().isChecked()
        self.batch_dialog.tableWidget_Brillouin.setEnabled(
            self.batch_config["peak-selection"]["select"]
        )
        self.batch_dialog.tableWidget_Rayleigh.setEnabled(
            self.batch_config["peak-selection"]["select"]
        )
    ...
    def on_evaluation_evaluate(self):
        ...
        self.batch_dialog.nrBrillouinPeaksGroup.setEnabled(
            self.batch_config["evaluation"]["evaluate"]
        )
        self.batch_dialog.bounds_table.setEnabled(
            self.batch_config["evaluation"]["evaluate"]
    ...
    def evaluate_batch_file(self, file):
        ...
        for rep_key in rep_keys:
            ...
            # Calibration
            cfg_calibration = self.batch_config["calibration"]
            if cfg_calibration["find-peaks"] or cfg_calibration["calibrate"]:
                self.tabWidget.setCurrentIndex(2)
                if self.aborted(file):
                    return
                QtCore.QCoreApplication.instance().processEvents()
                if not cfg_calibration["find-peaks"]:
                    self.widget_calibration_view.calibrate_all(do_not="find_peaks")
                elif not cfg_calibration["calibrate"]:
                    self.widget_calibration_view.calibrate_all(do_not="calibrate")
                else:
            ...
            if cfg_evaluation["evaluate"]:
                ...
                evc.set_nr_brillouin_peaks(cfg_evaluation["nr_brillouin_peaks"])
    ...
    def batch_add_files(self):
        folder_name = QFileDialog.getExistingDirectory(
            self, "Select folder...", directory=self.settings.value("path/last-used")
        )
        ...
        for h5_file in h5_files:
            ...
            if is_source_file(h5_file) and h5_file_md5 not in self.batch_files:
    ...
    def update_batch_file_table(self):
        ...
        success_icon_path = str(pathlib.Path(self.imdir) / "check_circle_outline.svg")
...
def excepthook(etype, value, trace):
    ...
    # Use signal to ensure the dialog is created on the main thread
    # This is required on macOS where GUI operations must happen on the main thread
    error_handler = get_error_handler()
    error_handler.error_occurred.emit(exception)
