...
class EvaluationView(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.combobox_parameter.currentIndexChanged.connect(self.on_select_parameter)
        self.combobox_peak_number.currentIndexChanged.connect(self.on_select_parameter)

        self.aspect_ratio.clicked.connect(self.refresh_plot)
        ...
        self.nrBrillouinPeaks_1.toggled.connect(lambda: self.setNrBrillouinPeaks(1))
        self.nrBrillouinPeaks_2.toggled.connect(lambda: self.setNrBrillouinPeaks(2))
        self.nrBrillouinPeaks_4.toggled.connect(lambda: self.setNrBrillouinPeaks(4))
    ...
    def on_click_image(self, event):
        ...
        for ind, p_ind in enumerate(idx):
            dslice = [
                slice(None) if p_ind == i else 0 for i, res in enumerate(resolution)
            ]
            r = abs(positions[p_ind][tuple(dslice)] - click_pos[ind])
            indices[p_ind] = int(np.argmin(r))

        # Convert indices to key
        image_key = self.evaluation_controller.get_key_from_indices(
            resolution, *indices
        )
        ...
        if image is not None:
            if isinstance(self.isd_image_map, matplotlib.image.AxesImage):
                ...
                self.isd_image_colorbar = self.isd_image_canvas.get_figure().colorbar(
                    self.isd_image_map
                )
        ...
        if spectra is not None and frequencies is not None:
            ...
            brillouin_fits, rayleigh_fits = self.evaluation_controller.get_fits(
                image_key
            )

            # Show the measured data
            self.isd_spectrum_plot.plot(
                1e-9 * frequencies[0], spectrum, color="tab:blue"
            )
            ...
            if pm is not None and evm is not None:
                ...
                for region_nr in range(brillouin_fits[0].shape[1]):
                    ...
                    for peak_nr in range(brillouin_fits[0].shape[2]):
                        ...
                        if peak_nr == 0 or peak_nr == (brillouin_fits[0].shape[2] - 1):
                            self.isd_spectrum_plot.plot(
                                1e-9 * x, y + brillouin_fits[3][idx], color=color
                            )
                ...
                for region_nr in range(rayleigh_fits[0].shape[1]):
                    ...
                    self.isd_spectrum_plot.plot(1e-9 * x, y, color="tab:purple")

        self.isd_spectrum_plot.grid(True, alpha=0.3)
        self.isd_spectrum_plot.minorticks_on()
        self.isd_spectrum_plot.grid(True, which="minor", alpha=0.15, linestyle=":")
        from matplotlib.ticker import MultipleLocator

        self.isd_spectrum_plot.xaxis.set_minor_locator(MultipleLocator(0.25))
    ...
    def open_image_spectrum(self):
        if self.image_spectrum_dialog is None:
            ...
            self.image_spectrum_dialog.setWindowTitle("Camera image & spectrum")
            ...
            self.isd_image_canvas = MplCanvas(
                self.image_spectrum_dialog.image_widget, toolbar=("Home", "Pan", "Zoom")
            )
            self.isd_image_plot = self.isd_image_canvas.get_figure().add_subplot(111)
            self.isd_image_map = None
            self.isd_image_colorbar = None
            ...
            self.isd_spectrum_plot = self.isd_spectrum_canvas.get_figure().add_subplot(
                111
            )
    ...
    def boundsChanged(self, row, column):
        ...
        if evm.bounds_w0 is not None:
            if column < 2:
                evm.bounds_w0[row][column] = self.bounds_table.item(row, column).text()
            elif column < 4:
                evm.bounds_fwhm[row][column - 2] = self.bounds_table.item(
                    row, column
                ).text()
    ...
    def reset_ui(self):
        ...
        if (
            self.image_spectrum_dialog is not None
    ...
    def refresh_ui(self):
        ...
        # we stop the timer
        if (
            self.evaluation_abort.value
            or self.max_count.value < 0
            or self.count.value >= self.max_count.value
        ):
    ...
    def refresh_plot(self):
        ...
        data, positions, dimensionality, labels = self.evaluation_controller.get_data(
        ...
        try:
            if dimensionality == 0:
                ...
                self.plot.axis("auto")
                self.plot.set_ylim(tuple(data[tuple(dslice)] * np.array([0.99, 1.01])))
            if dimensionality == 1:
                # If this is a line plot already, just set new data
                if isinstance(self.image_map, list) and isinstance(
                    self.image_map[0], matplotlib.lines.Line2D
                    ...
                    self.image_map[0].set_data(
                        positions[idx[0]][tuple(dslice)], data[tuple(dslice)]
                    )
                else:
                    self.clear_plots()
                ...
                with warnings.catch_warnings():
                    ...
                    if value_min < value_max:
                        self.plot.set_ylim(value_min, value_max)
            if dimensionality == 2:
                ...
                if isinstance(self.image_map, matplotlib.image.AxesImage):
                    logger.info("Updating existing image map (A)")
                    self.image_map.set_data(image_map)
                    self.image_map.set_extent(extent)
                else:
                    logger.info("Updating existing image map (B)")
                    self.clear_plots()
                    self.image_map = self.plot.imshow(
                        image_map, interpolation="nearest", extent=extent
                    )
                    self.colorbar = self.mplcanvas.get_figure().colorbar(self.image_map)
                ...
                with warnings.catch_warnings():
                    ...
                    (value_min, value_max) = self.get_plot_limits(data)
                    logger.info(f"Setting color limits: {value_min} - {value_max}")
